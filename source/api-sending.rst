...
.. container:: ptable
 ...
 o\:time-zone-localize             String should be set to preferred delivery time in ``HH:mm`` or ``hh:mm aa`` format, where ``HH`` is used for 24 hour format without AM/PM and ``hh`` is used for 12 hour format with AM/PM. *Please note that TZO is only available on certain plans. See www.mailgun.com/pricing for more info.*
